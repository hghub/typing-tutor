...
const CONTENT_OUTPUT_DIR = path.join(DATA_DIR, 'blogContent')
const INDEX_OUTPUT_FILE = path.join(DATA_DIR, 'blogIndex.js')
const LOADERS_OUTPUT_FILE = path.join(DATA_DIR, 'blogContentLoaders.js')
const RUNTIME_OUTPUT_FILE = path.join(DATA_DIR, 'blogPosts.js')
const PRERENDER_OUTPUT_DIR = path.join(REPO_ROOT, 'scripts', 'generated')
const PRERENDER_OUTPUT_FILE = path.join(PRERENDER_OUTPUT_DIR, 'blogPrerenderData.mjs')
...
function buildMetadataPost(post) {
  return {
    slug: post.slug,
    title: post.title,
...
function buildBlogIndexModule(posts) {
  const rows = posts.map(post => `  ${JSON.stringify(buildMetadataPost(post), null, 2).replace(/\n/g, '\n  ')}`)
...
function buildLegacyRuntimeModule(posts) {
...
function buildPostContentModule(post) {
  return `// Generated by scripts/sync-blog-posts.mjs\n// Do not edit this file directly. Edit markdown files in content/blog instead.\n\nconst postContent = ${escapeJsString(post.content)}\n\nexport default postContent\n`
}

function buildContentLoadersModule(posts) {
  const rows = posts.map(post => {
    const importPath = `./blogContent/${sanitizeSegment(post.section)}/${sanitizeSegment(post.slug)}.js`
    return `  ${JSON.stringify(post.slug)}: () => import(${JSON.stringify(importPath)})`
  })

  return `// Generated by scripts/sync-blog-posts.mjs\n// Do not edit this file directly. Edit markdown files in content/blog instead.\n\nexport const BLOG_CONTENT_LOADERS = {\n${rows.join(',\n')}\n}\n`
}

function buildPrerenderModule(posts) {
...
async function syncFromMarkdown() {
  ...
  await mkdir(CONTENT_OUTPUT_DIR, { recursive: true })
  await mkdir(PRERENDER_OUTPUT_DIR, { recursive: true })

  for (const post of posts) {
    const sectionDir = path.join(CONTENT_OUTPUT_DIR, sanitizeSegment(post.section))
    const postFile = path.join(sectionDir, `${sanitizeSegment(post.slug)}.js`)
    await mkdir(sectionDir, { recursive: true })
    await writeFile(postFile, buildPostContentModule(post), 'utf8')
  }

  await writeFile(INDEX_OUTPUT_FILE, buildBlogIndexModule(posts), 'utf8')
  await writeFile(LOADERS_OUTPUT_FILE, buildContentLoadersModule(posts), 'utf8')
  await writeFile(RUNTIME_OUTPUT_FILE, buildLegacyRuntimeModule(posts), 'utf8')
  await writeFile(PRERENDER_OUTPUT_FILE, buildPrerenderModule(posts), 'utf8')
